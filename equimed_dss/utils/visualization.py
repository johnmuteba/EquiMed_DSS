...
def plot_figure2_reliability_dashboard(
    ...
    colors = ["green" if v > 0.75 else "orange" if v > 0.6 else "red" for v in icc_values]
    ...
    ax1.set_title("A. Intraclass Correlation Coefficient", fontsize=14, fontweight="bold")
    ax1.legend()
    ...
    ax2.errorbar([alpha_val], ["Cronbach's α"], xerr=[[alpha_val - ci_lower], [ci_upper - alpha_val]],
    ...
    ax2.set_title("B. Internal Consistency (Cronbach's α)", fontsize=14, fontweight="bold")
    ax2.legend()
    ...
    ax3.axhline(mean_diff, color="black", linestyle="-", linewidth=2, label=f"Mean: {mean_diff:.3f}")
    ax3.axhline(loa_upper, color="red", linestyle="--", linewidth=2, label=f"Upper LoA: {loa_upper:.3f}")
    ax3.axhline(loa_lower, color="red", linestyle="--", linewidth=2, label=f"Lower LoA: {loa_lower:.3f}")
    ax3.fill_between([means.min(), means.max()], loa_lower, loa_upper, alpha=0.1, color="red")
    ...
    ax4.plot(timepoints, scores, marker="o", linewidth=2, markersize=8, color="steelblue")
    ...
    ax4.set_title("D. Temporal Stability of Reliability", fontsize=14, fontweight="bold")
    ax4.legend()
...
def plot_figure3_corpus_comparison(
    ...
    colors_corpus = {"Peer-Reviewed": "steelblue", "Community": "orange", "MIMIC-IV": "green"}
    ...
    bars = ax1.bar(corpora, gini_values, color=[colors_corpus.get(c, "gray") for c in corpora], alpha=0.7)
    ax1.axhline(0.3, color="red", linestyle="--", linewidth=2, label="High Inequality (>0.3)")
    ...
    for corpus_name in corpora_names:
        ...
        if key in temporal:
            ax2.plot(timepoints, temporal[key], marker="o", linewidth=2,
                    label=corpus_name, color=colors_corpus[corpus_name])

    ax2.axhline(0.1, color="red", linestyle="--", linewidth=2, label="Stability Threshold (0.1)")
    ax2.set_xlabel("Time Point", fontsize=12)
    ax2.set_ylabel("Temporal Fairness Drift (TFD)", fontsize=12)
    ax2.set_title("B. Temporal Fairness Drift Over Time", fontsize=14, fontweight="bold")
    ax2.legend()
    ...
    bp = ax3.boxplot(harm_values, labels=harm_labels, patch_artist=True,
    ...
    ax3.axhline(0.15, color="red", linestyle="--", linewidth=2, label="High Harm (>0.15)")
    ax3.set_ylabel("Harm-Adjusted Fairness Gap (HAFG)", fontsize=12)
    ax3.set_title("C. Clinical Harm Distribution by Corpus", fontsize=14, fontweight="bold")
    ax3.legend()
    ...
    for idx, metric_name in enumerate(metrics):
        ...
        values = [network_stability[c.lower().replace("-", "_")][metric_key]
                 for c in corpora_names if c.lower().replace("-", "_") in network_stability]
    ...
    ax4.set_title("D. Network Stability Characteristics", fontsize=14, fontweight="bold")
...
def plot_figure4_temporal_robustness(
    ...
    ci_lower = [mediation["indirect_ci_lower"], mediation["direct_ci_lower"], mediation["total_ci_lower"]]
    ci_upper = [mediation["indirect_ci_upper"], mediation["direct_ci_upper"], mediation["total_ci_upper"]]

    colors_med = ["coral", "steelblue", "purple"]
    bars = ax1.barh(effects, values, color=colors_med, alpha=0.7)

    # Add error bars
    for i, (effect, val, lower, upper) in enumerate(zip(effects, values, ci_lower, ci_upper)):
        ax1.errorbar([val], [i], xerr=[[val - lower], [upper - val]],
                    fmt="o", color="black", capsize=5, capthick=2)

    ax1.axvline(0, color="black", linestyle="-", linewidth=1)
    ax1.set_xlabel("Effect Size (β)", fontsize=12)
    ax1.set_title("A. Mediation Analysis\n(72.1% Indirect Pathway)", fontsize=14, fontweight="bold")
    ax1.grid(True, alpha=0.3, axis="x")
    ...
    ax2.errorbar(coefficients, y_pos,
                xerr=[[coefficients[i] - ci_lower_reg[i] for i in range(len(coefficients))],
    ...
    ax2.set_title("B. Regression Analysis with Bootstrap CIs", fontsize=14, fontweight="bold")
    ax2.grid(True, alpha=0.3, axis="x")
    ...
    ax3.plot(perturbation_levels, rcs_scores, marker="o", linewidth=3,
            markersize=8, color="steelblue", label="RCS Score")
    ax3.fill_between(perturbation_levels, 0, rcs_scores, alpha=0.2, color="steelblue")
    ax3.axhline(0.8, color="green", linestyle="--", linewidth=2, label="Deployment Ready (>0.8)")
    ax3.axhline(0.6, color="orange", linestyle="--", linewidth=2, label="Caution (0.6-0.8)")
...
def plot_figure5_ethics_governance(
    ...
    # ERI line
    line1 = ax1.plot(timepoints, eri_scores, marker="o", linewidth=2,
                     markersize=8, color="red", label="ERI Score")
    ax1.axhline(0.15, color="red", linestyle="--", linewidth=2, alpha=0.5, label="High Risk (>0.15)")
    ax1.fill_between(timepoints, 0, eri_scores, alpha=0.2, color="red")

    # Violations bars
    line2 = ax1_twin.bar(timepoints, violations, alpha=0.3, color="orange", label="Violations")
    ...
    categories = ["Risk\nManagement", "Accountability", "Monitoring", "Safety\nProtocols"]
    ...
    ax2.plot(angles, target, "o--", linewidth=2, color="green", alpha=0.5, label="Target (0.9)")
    ...
    nx.draw_networkx_nodes(G, pos, node_color=node_colors, node_size=800,
                          cmap="YlOrRd", vmin=0, vmax=max(node_colors), ax=ax3)
    nx.draw_networkx_labels(G, pos, labels={i: components[i] for i in range(len(components))},
                           font_size=8, font_weight="bold", ax=ax3)
    nx.draw_networkx_edges(G, pos, alpha=0.3, width=2, edge_color="gray", ax=ax3)

    ax3.set_title("C. Fairness Ecosystem Interconnections", fontsize=14, fontweight="bold")
    ax3.axis("off")
...
def plot_figure6_metric_networks(
    ...
    fig.suptitle("Metric Correlation Networks Across Corpora", fontsize=16, fontweight="bold")
    ...
    for corpus_key, title, ax in corpora:
        ...
        nx.draw_networkx_nodes(G, pos, node_color=node_colors, node_size=500,
                              cmap="viridis", vmin=0, vmax=max(node_colors) if node_colors else 1,
                              ax=ax)
        ...
        nx.draw_networkx_edges(G, pos, edge_color=weights, width=2,
...
def plot_figure7_intersectional_heatmap(
    ...
    gs = GridSpec(3, 3, figure=fig, width_ratios=[0.2, 1, 0.2], height_ratios=[0.2, 1, 0.05],
                  hspace=0.05, wspace=0.05)

    fig.suptitle("Intersectional Fairness Analysis with Pareto-Optimality",
    ...
    pareto_optimal = np.array(intersectional_data.get("pareto_optimal",
                              np.zeros_like(fairness_matrix, dtype=bool)))
    ...
    for i in range(len(row_labels)):
        for j in range(len(col_labels)):
            ...
            text = ax_main.text(j, i, f"{value:.2f}", ha="center", va="center",
    ...
    marginal_col = intersectional_data.get("marginal_col",
                                           np.mean(fairness_matrix, axis=0))
    ...
    marginal_row = intersectional_data.get("marginal_row",
                                           np.mean(fairness_matrix, axis=1))
    ...
    legend_elements = [Rectangle((0, 0), 1, 1, fill=False, edgecolor="blue",
