...
class EquiMedDataset:
    ...
    def from_csv(cls, filepath: str, **kwargs):
        ...
        read_csv_args = {
            ...
            if k in ["sep", "header", "index_col", "usecols"]
        }
    ...
    def to_group_metrics(self, metric_fn: Any, sensitive_col: str) -> Dict[str, float]:
        ...
        for group in groups:
            ...
            if len(group_data) > 0:
                score = metric_fn(
                    group_data[self.target_col], group_data[self.prediction_col]
                )
    ...
    def to_time_series(self, metric_fn: Any, freq: str = "M") -> List[float]:
        ...
        for _, period_data in self.df.set_index(self.timestamp_col).resample(freq):
            if len(period_data) > 0:
                score = metric_fn(
                    period_data[self.target_col], period_data[self.prediction_col]
                )
    ...
    def to_error_dicts(
        self, sensitive_col: str, group1: str, group2: str
        ...
        return {"group1": get_errors(g1_data), "group2": get_errors(g2_data)}
