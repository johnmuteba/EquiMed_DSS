...
class IntersectionalBiasScore:
    ...
    def calculate_subgroup_similarity(self, subgroup_vectors: Dict[str, np.ndarray]) -> Dict[str, Any]:
        """
    ...
    def interaction_analysis(self, df: pd.DataFrame, formula: str = 'score ~ C(race) * C(gender) * C(ses)') -> Dict[str, float]:
        """
        ...
        for col in ['race', 'gender', 'ses']:
            if col in df.columns:
                ...
                ss_between = sum(df[df[col]==g].shape[0] * (mean - grand_mean)**2 for g, mean in group_means.items())
        ...
        if 'race' in df.columns and 'gender' in df.columns:
            ...
            grand_mean = df['score'].mean()
            ss_between = sum(df[df['race_gender']==g].shape[0] * (mean - grand_mean)**2 for g, mean in group_means.items())
            ss_total = sum((df['score'] - grand_mean)**2)
            effects['race_gender_interaction'] = (ss_between / ss_total) - effects.get('race', 0) - effects.get('gender', 0)
